...
def compute_cost_with_reg(Y, Yhat, parameters, lambd):
    """
    ...
    cross_entropy_cost = compute_cost(Y, Yhat)

    # sum of squares W
    temp = 0
    for l in range(L):
        W = parameters["W" + str(l+1)]
        temp = temp + np.square(W).sum()
...
def backpropagation(Y, Yhat, cache, parameters, lambd):
    """
    ...
    dZ = A - Y
    dW = 1/m * np.dot(dZ, cache["A" + str(l-1)].T) + (lambd/m) * W
...
def model(X, Y, layers_dims, lambd, learning_rate = 0.0075, num_iterations = 3000, print_cost=False):
    """
    ...
    for i in range(0, num_iterations):
        ...
        # Compute cost.
        cost = compute_cost_with_reg(Y, Yhat, parameters, lambd)
    
        # Backward propagation.
        grads = backpropagation(Y, Yhat, cache, parameters, lambd)
